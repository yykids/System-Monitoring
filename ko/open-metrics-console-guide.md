...
### 수집 대상 추가
수집 대상 관리 팝업의 상단에는 모니터링 대상 추가에 필요한 입력란이 표시됩니다. **서버** 입력란을 클릭하면 현재 System Monitoring의 Agent가 정상적으로 동작하는 인스턴스들이 표시되며, 하나 이상을 선택하여 수집할 대상을 지정할 수 있습니다.
...
알림 설정의 구분을 위하여 **이름**을 입력하고, 감시 대상의 **임계치**를 입력합니다. 설정한 임계치는 해당 차트의 지표로 추가한 항목들 전체에 일괄 적용됩니다. **지속시간**은 해당 조건이 몇 분 이상 유지될 경우 이벤트가 발생한 것으로 판단할지 설정하는 것입니다. 만약 이 값을 0으로 입력한다면 주기마다 수집된 값이 해당 조건을 만족하는 즉시 이벤트가 발생한 것으로 처리합니다.
