...
선택한 지표가 (라벨/레이블)을 가지고 있는 경우 그것의 값을 비교하여 조건에 맞는 데이터만 조회할 수 있도록 **필터**를 설정할 수 있습니다. 해당 지표가 가지고 있는 (라벨/레이블) 중 하나를 선택하고 비교할 값을 입력합니다. 필터 조건은 여러 개의 (라벨/레이블)에 대해 지정할 수 있지만, 같은 (라벨/레이블)에 대해 중복으로 설정하는 것은 불가능합니다. 제공하고 있는 비교 방법은 다음과 같습니다.


연산자 | 의미 | 예시
-------- | ------- | -------
`=` | 일치함 | `mode = user`<br/>`mode` (라벨/레이블)의 값이 `user`인 것만 표시
`!=` | 일치하지 않음 | `mode != idle`<br/>`mode` (라벨/레이블)의 값이 `idle`이 아닌 것만 표시
`=~` | 정규 표현식에 부합함 | `mode =~ user|system`<br/>`mode` (라벨/레이블)의 값이 `user` 또는 `system`인 것만 표시
`!~` | 정규 표현식에 부합하지 않음 | `code !~ 5.*`<br/>`code` (라벨/레이블)의 값이 `5`로 시작하지 않는 것만 표시
